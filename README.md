...
> (currently a private repository). This archive remains available for posterity.
...
| **Phi Vox Oscillator** | VOSIM voice oscillator (Kaegi & Tempelaars, Institute of Sonology — JAES 1978): each cycle emits a burst of raised-cosine pulses per formant branch, decay-scaled, then silence — two branches make a vowel, and formants are fixed in Hz so vocal identity survives transposition. Zones anchor on eight voice characters (Breath…Rasp) with phi-triangle wander far beyond vocal physiology (growing-pulse rasp, alternating-polarity hollowness). The A/B crossfade is a diphthong, and crossfade motion injects voiced-gated consonant bursts — the wave index knob makes it speak. A Formant Track menu item (0–50) blends fixed-Hz formants toward note-relative ratios (harmonic-locked overtone behavior), a slow φ-triangle shifts both formants coherently across zone space (the same vowel findable higher/lower), and formant frequencies glide with ~20ms portamento. Stateless per voice (zero buffers, ~260 bytes per Sound). See [VOSIM design notes](docs/dev/phi-vox-vosim.md) and [testing guide](docs/features/phi-vox-testing.md). |
...
| **Phi Gendy Oscillator** | Xenakis dynamic stochastic synthesis (GENDYN lineage — Formalized Music; Serra CMJ 1993; Hoffmann CMJ 2000) with locked pitch: a 16-breakpoint polygon whose amplitudes perform the GENDYN double random walk between elastic barriers, once per buffer, scanned at the note pitch. Phi-triangle landscapes paint per-node entropy and cage terrain (one waveform region boils while another stays calm); a home-shape spring gives calm zones a timbre to relax into; crossfade motion STARTLES the walkers and the spring reels them home. Zones Haze…Frenzy trace an exponential entropy arc from barely-alive warmth to reflection-grit. Rich saw-class spectra, PHI_MORPH-class cost, zero per-voice state. See [design notes](docs/dev/phi-gendy-stochastic.md) and [testing guide](docs/features/phi-gendy-testing.md). |
| **Phi Stair Oscillator** | Staircase wavetable (stepped-waveform lineage: Walsh-function synthesis — Chamberlin 1980, Hutchins' Electronotes): up to 16 steps whose heights follow zone pattern families (Brick, Terrace, Mesa, φ-random Glyph, Teeth…), widths carry asymmetry landscapes, and a slope dimension sweeps hard-edged pulse-class brightness into ramped mellowness. Step count is continuous (fractional last step — sweeps never pop), loudness is statically normalized per morph position, and the stereo zone knob works via dual staircase taps. Zero per-voice state. See [design notes](docs/dev/phi-stair-steps.md) and [testing guide](docs/features/phi-stair-testing.md). |
| **Phi Stereo Zones** | Zone-style stereo control shared by the phi oscillators (Stereo item in the oscillator menu): position within each of eight zones sets the stereo amount (every zone starts mono), the zone selects the character — PhiWeave/PhiStair dual pickup taps (distance, tonal tilt, counter-scan), PhiGendy lagged polygon taps (~3ms micro-slap on walk events), PhiSwarm slave separation with beat-opposition autopan. Zones Slim…Vast. Works with unison off; composes with unison stereo spread. PhiVox excluded. |
...
- [Phi Gendy Stochastic Notes](docs/dev/phi-gendy-stochastic.md) - Xenakis dynamic stochastic synthesis: the double random walk, elastic barriers, phi terrain and the startle morph, with figures and original literature
- [Phi Stair Step Notes](docs/dev/phi-stair-steps.md) - Staircase wavetable: pattern families, the slope dimension, continuous step count, and the Walsh-function lineage, with figures
