...
- **Multi-clip coupling when finite-repeat clips have different loopLengths** — current implementation uses the global `distanceTilLaunchEvent` accumulated across all finite-repeat clips (max-aggregation), so e.g. a 1-beat clip running alongside a 4-beat clip gets its counter ticked at the 4-beat boundary instead of its own 1-beat boundary. Fixing properly needs either per-clip scheduled-event infrastructure separate from the single `launchEventAtSwungTickCount` slot, or direct state mutation at `Clip::processCurrentPos` loop wraps — the latter was tried (commit `669f7027`, reverted) and ran afoul of Deluge's per-clip tick-processing invariants. Good slice works for single finite-repeat clip per song, or multiple finite-repeat clips with the same loopLength. Documented as known limitation.
