...
SineShaperTwistParams computeSineShaperTwistParams(q31_t twist, const SineTableShaperParams* ssParams) {
	...
	if (phRaw != 0.0) {
		...
		// Rect: unipolar, scale(φ^0.67) * param(φ^1.0)
		float rS = triangleSimpleUnipolar(pos * phi::kPhi067 * fmR + ph067 + 0.910f, 0.5f);
		float rP = triangleSimpleUnipolar(pos * phi::kPhi100 * fmR + ph100 + 0.845f, 0.5f);
		result.rectAmount = rS * rP;
	...
	if (twist < kZone1) {
		...
		// Zone 2: Rect - unipolar triangle (duty=1.0: dry→rect→dry)
		float pos = static_cast<float>(twist - kZone2) / static_cast<float>(kZone3 - kZone2);
		result.rectAmount = triangleSimpleUnipolar(pos, 1.0f);
	}
...
q31_t sineShapeCore(q31_t input, float driveGain, int32_t zone, const ShaperWeights* zoneWeights, float evenAmount,
                    float evenDryBlend, float rectAmount) {
	...
	if (zone == 7) {
		...
		q31_t scaledInput = lshiftAndSaturateUnknown(driven, 7);
		...
		q31_t scaledInput = lshiftAndSaturateUnknown(driven, 8);
		...
		constexpr float kChebyGainScaled = kChebyBoost * kMinus3dB * static_cast<float>(ONE_Q31);

		float scaled = result * kChebyGainScaled;
		shaped = static_cast<q31_t>(std::clamp(scaled, -2147483648.0f, 2147483647.0f));
	}
		...
		// Shared: use pre-converted q31 weights (updated at stride rate in buffer loop)
		q31_t w0q = zoneWeights->c3q;
		q31_t w1q = zoneWeights->c5q;
		q31_t w2q = zoneWeights->c7q;
		q31_t w3q = zoneWeights->c9q;

		// Shared: apply input gain and compute base phase
		q31_t gained = multiply_32x32_rshift32(driven, zoneWeights->c1q) << 2;
		uint32_t phase1 = static_cast<uint32_t>(gained) << 8;
		q31_t sine1 = getSine(phase1);
		// Peak scaling: all modes target ~0.5 peak to match ring mod's natural 0.5
		// Ring mod (zone 4) gets 0.5 from multiply_32x32_rshift32(a, b) ≈ a*b/2
		// Other zones use >> 1 (equivalent to multiply_32x32_rshift32(x, ONE_Q31))
		...
		if (zone == 2) {
			...
			m0 = ((sine1 >> 1) + (sine2 >> 1)) >> 1;
			m1 = multiply_32x32_rshift32(sine1, getSine(phase2 + 0x40000000u));
			m2 = getSine(phase1 + static_cast<uint32_t>(sine2 >> 1)) >> 1;
			m3 = getSine(static_cast<uint32_t>(sine1) << 1) >> 1;
		}
		else if (zone == 3) {
			// Fold: k=1,2,3,4 (increasing fold depth) - all peak at 0.5
			m0 = getSine(static_cast<uint32_t>(sine1)) >> 1;
			m1 = getSine(static_cast<uint32_t>(sine1) << 1) >> 1;
			m2 = getSine(static_cast<uint32_t>(sine1) + (static_cast<uint32_t>(sine1) << 1)) >> 1;
			m3 = getSine(static_cast<uint32_t>(sine1) << 2) >> 1;
		}
		else if (zone == 4) {
			...
			m0 = getSine(phase1 + static_cast<uint32_t>(mod >> 2)) >> 1;
			m1 = getSine(phase1 + static_cast<uint32_t>(mod >> 1)) >> 1;
			m2 = getSine(phase1 + static_cast<uint32_t>((mod >> 1) + (mod >> 2))) >> 1;
			m3 = getSine(phase1 + static_cast<uint32_t>(mod)) >> 1;
		}
	...
	// === Even + Rect post-processing (fused float domain) ===
	// When either is active, convert to float once, process both, convert back once.
	// Saves redundant q31↔float conversions when both are active simultaneously.
	bool hasEven = (evenAmount > 0.0f || evenDryBlend > 0.0f);
	bool hasRect = (rectAmount > 0.0f);
	...
	if (hasEven || hasRect) {
		...
		if (hasEven) {
			if (shapedF > 0.0f && evenAmount > 0.0f) {
				...
				shapedF = ratio * shapedF * kEvenGain;
			}
			else if (shapedF < 0.0f && evenDryBlend > 0.0f) {
				float absShapedF = -shapedF;
				float absDrivenF = std::abs(static_cast<float>(driven));
				float cappedDrivenAbs = std::min(absDrivenF, absShapedF * 2.0f);
				shapedF = -(absShapedF * (1.0f - evenDryBlend) + cappedDrivenAbs * evenDryBlend);
			}
		}
		...
		if (hasRect) {
			...
			shapedF = shapedF + (absVal - shapedF) * rectAmount;
		}

		shaped = static_cast<q31_t>(shapedF);
	}
...
StereoShaped sineShapeCoreStereo(q31_t inputL, q31_t inputR, float driveGainL, float driveGainR, int32_t zone,
	...
	if (zone == 7) {
		// === Zone 7: Polynomial waveshaping with cascade blend + self-multiplication ===
		q31_t scaledInputL = lshiftAndSaturateUnknown(drivenL, 7);
		q31_t scaledInputR = lshiftAndSaturateUnknown(drivenR, 7);
		...
		q31_t scaledInputL = lshiftAndSaturateUnknown(drivenL, 8);
		q31_t scaledInputR = lshiftAndSaturateUnknown(drivenR, 8);
		...
		float scaledL = resultL * kChebyGainScaled;
		float scaledR = resultR * kChebyGainScaled;
		shapedL = static_cast<q31_t>(std::clamp(scaledL, -2147483648.0f, 2147483647.0f));
		shapedR = static_cast<q31_t>(std::clamp(scaledR, -2147483648.0f, 2147483647.0f));
	}
	else {
		// === Zones 2-6: FM synthesis ===
		// Use pre-converted q31 weights (updated at stride rate in buffer loop)
		q31_t w0qL = zoneWeightsL->c3q;
		q31_t w1qL = zoneWeightsL->c5q;
		q31_t w2qL = zoneWeightsL->c7q;
		q31_t w3qL = zoneWeightsL->c9q;

		q31_t w0qR = zoneWeightsR->c3q;
		q31_t w1qR = zoneWeightsR->c5q;
		q31_t w2qR = zoneWeightsR->c7q;
		q31_t w3qR = zoneWeightsR->c9q;

		// Compute base phases
		q31_t gainedL = multiply_32x32_rshift32(drivenL, zoneWeightsL->c1q) << 2;
		q31_t gainedR = multiply_32x32_rshift32(drivenR, zoneWeightsR->c1q) << 2;
		...
		// All modes target ~0.5 peak: ring mod gets this from multiply, others use >> 1
		if (zone == 2) {
			...
			m0L = ((sine1L >> 1) + (sine2L >> 1)) >> 1;
			m0R = ((sine1R >> 1) + (sine2R >> 1)) >> 1;
			...
			m2L = getSine(phase1L + static_cast<uint32_t>(sine2L >> 1)) >> 1;
			m2R = getSine(phase1R + static_cast<uint32_t>(sine2R >> 1)) >> 1;
			m3L = getSine(static_cast<uint32_t>(sine1L) << 1) >> 1;
			m3R = getSine(static_cast<uint32_t>(sine1R) << 1) >> 1;
		}
		else if (zone == 3) {
			// Fold: k=1,2,3,4
			m0L = getSine(static_cast<uint32_t>(sine1L)) >> 1;
			m0R = getSine(static_cast<uint32_t>(sine1R)) >> 1;
			m1L = getSine(static_cast<uint32_t>(sine1L) << 1) >> 1;
			m1R = getSine(static_cast<uint32_t>(sine1R) << 1) >> 1;
			m2L = getSine(static_cast<uint32_t>(sine1L) + (static_cast<uint32_t>(sine1L) << 1)) >> 1;
			m2R = getSine(static_cast<uint32_t>(sine1R) + (static_cast<uint32_t>(sine1R) << 1)) >> 1;
			m3L = getSine(static_cast<uint32_t>(sine1L) << 2) >> 1;
			m3R = getSine(static_cast<uint32_t>(sine1R) << 2) >> 1;
		}
		else if (zone == 4) {
			// Ring: n=2,3,4,5
			...
			m0L = ((sine1L >> 1) + (s2L >> 1)) >> 1;
			m0R = ((sine1R >> 1) + (s2R >> 1)) >> 1;
			m1L = ((sine1L >> 1) + (s3L >> 1)) >> 1;
			m1R = ((sine1R >> 1) + (s3R >> 1)) >> 1;
			m2L = ((sine1L >> 1) + (s4L >> 1)) >> 1;
			m2R = ((sine1R >> 1) + (s4R >> 1)) >> 1;
			m3L = ((sine1L >> 1) + (s5L >> 1)) >> 1;
			m3R = ((sine1R >> 1) + (s5R >> 1)) >> 1;
		}
		else { // zone == 6
			// Mod: FM depths
			q31_t modL = getSine(phase1L << 1);
			q31_t modR = getSine(phase1R << 1);
			m0L = getSine(phase1L + static_cast<uint32_t>(modL >> 2)) >> 1;
			m0R = getSine(phase1R + static_cast<uint32_t>(modR >> 2)) >> 1;
			m1L = getSine(phase1L + static_cast<uint32_t>(modL >> 1)) >> 1;
			m1R = getSine(phase1R + static_cast<uint32_t>(modR >> 1)) >> 1;
			m2L = getSine(phase1L + static_cast<uint32_t>((modL >> 1) + (modL >> 2))) >> 1;
			m2R = getSine(phase1R + static_cast<uint32_t>((modR >> 1) + (modR >> 2))) >> 1;
			m3L = getSine(phase1L + static_cast<uint32_t>(modL)) >> 1;
			m3R = getSine(phase1R + static_cast<uint32_t>(modR)) >> 1;
		}
	...
	// === Even + Rect post-processing (fused float domain) ===
	// Convert to float once, process both stages, convert back once.
	bool hasEven = (evenAmount > 0.0f || evenDryBlend > 0.0f);
	bool hasRect = (rectAmount > 0.0f);
	...
	if (hasEven || hasRect) {
		...
		if (hasEven) {
			...
			if (shapedFL > 0.0f && evenAmount > 0.0f) {
				...
				shapedFL = (1.0f - evenAmount * (1.0f - x)) * shapedFL * kEvenGain;
			}
			else if (shapedFL < 0.0f && evenDryBlend > 0.0f) {
				float absS = -shapedFL;
				float absD = std::abs(static_cast<float>(drivenL));
				shapedFL = -(absS * (1.0f - evenDryBlend) + std::min(absD, absS * 2.0f) * evenDryBlend);
			}

			// Right
			if (shapedFR > 0.0f && evenAmount > 0.0f) {
				float x = std::clamp(shapedFR * kNormFactor * kBoost, 0.0f, 1.0f);
				shapedFR = (1.0f - evenAmount * (1.0f - x)) * shapedFR * kEvenGain;
			}
			else if (shapedFR < 0.0f && evenDryBlend > 0.0f) {
				float absS = -shapedFR;
				float absD = std::abs(static_cast<float>(drivenR));
				shapedFR = -(absS * (1.0f - evenDryBlend) + std::min(absD, absS * 2.0f) * evenDryBlend);
			}
		}
		...
		if (hasRect) {
			// Left
			shapedFL = shapedFL + (std::abs(shapedFL) - shapedFL) * rectAmount;
			// Right
			shapedFR = shapedFR + (std::abs(shapedFR) - shapedFR) * rectAmount;
		}

		shapedL = static_cast<q31_t>(shapedFL);
		shapedR = static_cast<q31_t>(shapedFR);
	}
...
void sineShapeBuffer(std::span<q31_t> buffer, q31_t drive, q31_t* smoothedDrive, SineShaperVoiceState* voiceState,
	...
	// Wet/dry mix coefficient: hoisted before loop since mix is constant across buffer
	...
	bool isFmZone = (zone >= 2 && zone <= 6);
	if (isFmZone) {
		shaperWeights.updateQ31();
	}
	...
	for (auto& sample : buffer) {
		...
		q31_t shaped = sineShapeCore(inputWithFb, currentDriveGain, zone, &shaperWeights, twist.evenAmount,
		                             twist.evenDryBlend, twist.rectAmount);
		...
		// Wet/dry crossfade (invMix hoisted before loop)
...
void sineShapeBuffer(std::span<StereoSample> buffer, q31_t drive, q31_t* smoothedDrive,
	...
	bool isFmZone = (zone >= 2 && zone <= 6);
	if (isFmZone) {
		shaperWeightsL.updateQ31();
		shaperWeightsR.updateQ31();
	}
	...
	constexpr float kStereoDriveScale = 0.25f; // Up to 25% drive reduction at full width
	constexpr float kMinLfoHz = 0.05f;         // Slow minimum (~20s period)
	constexpr float kMaxLfoHz = 3.0f;          // Slow max for subtle stereo motion
	...
	// Hoist LFO to buffer rate: at max 6Hz/44.1kHz, phase changes ~0.017 per 128 samples
	// The per-buffer error is <2%, completely inaudible for stereo width modulation
	float bufferLfoValue = triangleFloat(lfoPhase, kStereoLfoDuty);
	// Pre-compute drive stereo reduction factor (0 when LFO >= 0 or stereo disabled)
	float driveStereoReduction = (bufferLfoValue < 0.0f) ? (-bufferLfoValue) * driveStereoScale : 0.0f;
	float driveRScale = 1.0f - driveStereoReduction;
	// Advance LFO phase for entire buffer
	lfoPhase += lfoPhaseIncrement * static_cast<float>(buffer.size());
	// Wrap phase (handles multiple wraps for safety, though at max rate only ~0.017 per buffer)
	lfoPhase -= std::floor(lfoPhase);

	for (auto& sample : buffer) {
		float driveGainL = currentDriveGain;
		float driveGainR = currentDriveGain * driveRScale;
		...
		StereoShaped shaped =
		    sineShapeCoreStereo(inputL, inputR, driveGainL, driveGainR, zone, &shaperWeightsL, &shaperWeightsR,
		                        twist.evenAmount, twist.evenDryBlend, twist.rectAmount);
		...
		// Wet/dry crossfade (invMix hoisted before loop)
		sample.l =
		    add_saturate(multiply_32x32_rshift32(sample.l, invMix) << 1, multiply_32x32_rshift32(hpfWetL, mix) << 1);
		sample.r =
		    add_saturate(multiply_32x32_rshift32(sample.r, invMix) << 1, multiply_32x32_rshift32(hpfWetR, mix) << 1);
	}
