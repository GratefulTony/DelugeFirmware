...
void Song::renderAudio(deluge::dsp::StereoBuffer<q31_t> outputBuffer, int32_t* reverbBuffer,
	...
	deluge::dsp::StereoSample<q31_t> tempBufferStorage[kMaxTempBufferSamples];
	...
	for (Output* output = firstOutput; output; output = output->next) {
		...
		if (output->shouldRenderInSong()) {
			...
			if (output == focusedOutputForRetro && outputBuffer.size() <= kMaxTempBufferSamples) {
				...
				std::memset(tempBufferStorage, 0, numSamples * sizeof(deluge::dsp::StereoSample<q31_t>));

				// Create a span for the temp buffer
				deluge::dsp::StereoBuffer<q31_t> tempBuffer(tempBufferStorage, numSamples);
