...
for i in range(2):
    byOsc[i] = [
        ...
        Menu(
            "osc::PhiMorphZone",
            f"osc{i}PhiZoneAMenu",
            ["{name}", "{title}", f"{i}", "0"],
            "oscillator/phi_morph_zone.md",
            name="STRING_FOR_PHI_ZONE_A",
            title="STRING_FOR_OSC_PHI_ZONE_A_MENU_TITLE",
            available_when="Oscillator type must be PhiMorph, PhiWeave, PhiVox, or PhiSwarm (the zone selects a waveform, physics terrain, voice character, or synchronization regime respectively)",
        ),
        Menu(
