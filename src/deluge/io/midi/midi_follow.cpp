...
void MidiFollow::handleReceivedCC(MIDICable& cable, ModelStackWithTimelineCounter& modelStackWithTimelineCounter,
	...
	if (baseCC != MIDI_CC_NONE && ccNumber >= baseCC && ccNumber < baseCC + kNumModButtons * kNumPhysicalModKnobs) {
		handleModKnobCC(cable, modelStackWithTimelineCounter, clip, ccNumber - baseCC, ccNumber, ccValue);
		return;
	}
...
void MidiFollow::handleModKnobCC(MIDICable& cable, ModelStackWithTimelineCounter& modelStackWithTimelineCounter,
                                 Clip* clip, int32_t slotIndex, int32_t ccNumber, int32_t ccValue) {
	if (!clip || !clip->output) {
		return;
	}
	...
	if (highResDivisor > 1) {
		...
		newKnobPos = MidiTakeover::calculateKnobPos(cable, knobPos, ccValue, nullptr, true, ccNumber, isStepEditing);
		shouldUpdate = (newKnobPos != knobPos);
		newValue = modelStackWithParam->paramCollection->knobPosToParamValue(newKnobPos, modelStackWithParam);
	}
